...
\begin{abstract}
We propose a foundational reconceptualization of risk science. Rather than treating risk as the product of static factors (hazard, exposure, vulnerability), we view risk as emerging from the structure of branching causal pathways through which systems evolve. This path-distributional perspective reveals that fragility arises not merely from having few viable futures, but from the topology of how those futures relate to each other. By representing possible futures as multiway causal graphs where nodes are complete system configurations, we make path dependence, structural emergence, and topological transitions visible and quantifiable. This framework provides a new epistemic foundation for understanding adaptive capacity, detecting approaching tipping points, and distinguishing apparent from genuine robustness.
\end{abstract}
...
In the risk context, the Multiway Evolution Graph maps the "space of possible futures"—showing which system states can transition to which others based on intial discrete rules and their associated actional functional value. The Multiway Causal Graph maps which prior decisions or failures are necessary prerequisites for a future event to occur.
...
In classical risk, we assign a risk value to a specific outcome (e.g., "Probability of Flood = 1\%"). In the multiway causal framework, risk is the \textit{shape} of the possibility space itself constrained by the action functional. It is a holistic geometric feature, not a local variable. 
...
In this example, the multiway causal graph is constructed using the unified three-state alphabet ${I, D, E}$, representing the states \emph{Intact}, \emph{Damaged}, and \emph{End}. Two illustrative systems are considered---a \textbf{Glass} system and a \textbf{Plant} system. Both operate on the same state space but differ in their transition rules, enabling a consistent comparison between fragility and resilience.

Starting from the initial state $I$, each applicable rule generates a new branch. The resulting multiway system represents all possible evolutions of the state up to a chosen depth. Nodes in the multiway graph correspond to states, and directed edges correspond to discrete events arising from rule applications.

Each event $e$ is associated with a decision-theoretic action functional: \begin{equation} j(e) = \mathrm{Effort}(e) - \mathrm{Utility}(e) + \mathrm{Constraints}(e). \end{equation} For any trajectory $\gamma = (e_1, e_2, \dots, e_n)$, the accumulated cost is \begin{equation} J[\gamma] = \sum_{t=1}^n j(e_t). \end{equation} This provides a quantitative means to compare evolutionary histories produced by different systems.

\paragraph{Glass.} The Glass rule-set emphasizes fragility: transitions tend to push the system toward the terminal state $E$. Several transitions accumulate additional constraint penalties, causing steeper increases in $J[\gamma]$.

\paragraph{Plant.} The Plant rule-set includes healing transitions $D \to I$, enabling partial recovery. This introduces richer branching dynamics and admits trajectories with lower cumulative cost.

The revised implementation produces: \begin{itemize} \item multiway state graphs, \item causal graphs capturing event dependencies, \item JSON files listing all events and trajectories, \item histograms of accumulated $J$ values. \end{itemize} These outputs supersede the earlier example and accurately reflect the rule sets, causal semantics, and action functional used in the study.
